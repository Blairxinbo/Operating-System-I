...
void lock(int thread) {
    ...
    int j;
    Entering[thread] = 1;
    mfence(); //Memory fence can prevent out of order issue around the fence.
              // For many threads on multiple cores, we need to make sure that other threads have been finished after the data struct modified.
              //
    Number[thread] = 1 + max((int *)Number);
    Entering[thread] = 0;
    mfence(); //We need to wait Entering[thread]=0, then we can pass the value to multiple cores.
    for (j = 0; j<= NUM_threads; j++){
        if (j != thread) {
            while(Entering[j]) {}
            while(Number[j] != 0 && ((Number[thread] > Number[j])
                                     || (Number[thread] == Number[j] && thread > j))){}
        }
